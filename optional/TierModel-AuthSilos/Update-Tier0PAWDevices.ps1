...
<#
.Synopsis
    This scripts is meant to maintain the Tier 0 PAW Devices group membership in order to support Authentication Silo policies

.DESCRIPTION
    The script will get all computer objects under the Tier 0 PAW Devices OU and add them to the Tier0PawDevices group. The group is part of an authentication silo policy.

.EXAMPLE
	.\Update-Tier0PAWDevices.ps1

.EXAMPLE
	.\Update-Tier0PAWDevices.ps1 -MultiDomainForest $true
        ...
        try {
            if ($null -eq (Get-ADObject "$OU,$((Get-ADDomain -Server $domain).DistinguishedName)" -Server $domain)){
                ...
                Foreach ($T0Computer in $T0computers){
                    ...
                    try {
                        Grant-ADAuthenticationPolicySiloAccess -Identity "*- Tier 0 Authentication Silo" -Account $T0Computer.DistinguishedName -Server $domain -ErrorAction SilentlyContinue | Out-Null
                    } catch { }
                }
            }
        }
