...
if ([string]::IsNullOrWhiteSpace($TieredUsersOU)) {
    $TieredUsersOU = "OU=Tier 1 Accounts,OU=Tier 1,OU=Tier Model Administration${ParentPrefix}"
}
if ([string]::IsNullOrWhiteSpace($TieredServiceAccountsOU)) {
    $TieredServiceAccountsOU = "OU=Tier 1 Service Accounts,OU=Tier 1,OU=Tier Model Administration${ParentPrefix}"
}
