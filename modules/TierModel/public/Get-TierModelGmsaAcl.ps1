function Get-TierModelGmsaAcl {
    <#
    .SYNOPSIS
    Analyze TierModel gMSA ACL delegation requirements and generate deployment plan.
    ...
    and delegation groups exist before evaluating ACL state.
    
    .PARAMETER Config
    TierModel configuration object containing gMSA ACL delegation definitions.
    ...
    Write-TierModelLog -Level Info -Message "GmsaAclPlanningStart" -Data @{
    ...
    try {
        ...
        Write-TierModelLog -Level Info -Message "Analyzing gMSA ACL delegations" -Data @{
        ...
        foreach ($ouPath in $uniqueOUs) {
            try {
                ...
                $planErrors += @{
                    ...
                    Message = "Target OU not found: '$ouPath'. Use -OusOnly to deploy the OUs, or use -FullDeployment -IncludeGmsa."
        ...
        foreach ($group in $uniqueGroups) {
            try {
                ...
                $planErrors += @{
                    ...
                    Message = "Delegation group '$group' not found. Use -GroupsOnly to deploy the Groups, or use -FullDeployment -IncludeGmsa."
            ...
            try {
                ...
                try {
                    ...
                    if ($existingAcl) {
                        ...
                        Write-TierModelLog -Level Info -Message "gMSA ACL delegation already exists with exact match" -Data @{ TargetOUPath = $targetOUPath; IdentityReference = $identityReference; ObjectType = $resolvedObjectType; Rights = ($acl.activedirectoryrights -join ', '); CorrelationId = $CorrelationId } | Out-Null
                    }
                } catch {
                    $planErrors += @{
                        Timestamp = Get-Date
                ...
                if ($needsApplication) {
                    ...
                    Write-TierModelLog -Level Info -Message "gMSA ACL delegation needs application" -Data @{
                ...
                $planErrors += @{
                    ...
                    Message = "Failed to analyze gMSA ACL delegation: $($_.Exception.Message)"
                    Context = @{
                        IdentityReference = $acl.identityreference
        ...
        Write-TierModelLog -Level Info -Message "GmsaAclPlanningComplete" -Data @{
            TotalActions = $actions.Count
            CreateActions = $createActions
            ValidationErrors = $planErrors.Count
        ...
        Write-TierModelLog -Level Error -Message "gMSA ACL planning failed" -Data @{
