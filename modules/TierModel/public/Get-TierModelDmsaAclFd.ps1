function Get-TierModelDmsaAclFd {
    <#
    .SYNOPSIS
    Analyze TierModel dMSA ACL delegation requirements and generate deployment plan (Full Deployment variant).
    ...
    by earlier phases while still resolving the domain-specific dMSA class GUID.
    #>
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
    ...
    Write-TierModelLog -Level Info -Message "DmsaAclFdPlanningStart" -Data @{
    ...
    try {
        ...
        Write-TierModelLog -Level Info -Message "Analyzing dMSA ACL delegations (Full Deployment)" -Data @{
            TotalAclsInConfig = $delegations.Count
            ServiceAccountClass = 'msDS-DelegatedManagedServiceAccount'
        ...
        foreach ($acl in $delegations) {
            try {
                    ...
                    try {
                        ...
                        if ($existingAcl) {
                            ...
                            if (-not $Silent) {
                                ...
                                Write-Host "  ✅ dMSA ACL Exists: $identityReference -> $ouName" -ForegroundColor Green
                            }
                            
                            Write-TierModelLog -Level Info -Message "dMSA ACL delegation already exists with exact match (Fd)" -Data @{
                        ...
                        Write-TierModelLog -Level Debug -Message "Could not read dMSA ACL, will plan create action" -Data @{
                            TargetOUPath = $targetOUPath
                            IdentityReference = $identityReference
                ...
                if ($needsApplication) {
                    ...
                    Write-TierModelLog -Level Info -Message "dMSA ACL delegation needs application (Fd)" -Data @{
                ...
                $planErrors += @{
                    Timestamp = Get-Date
                    Category = 'Processing'
                    Code = 'DmsaAclFdAnalysisFailed'
                    Message = "Failed to analyze dMSA ACL delegation (Fd): $($_.Exception.Message)"
                    Context = @{
                        IdentityReference = $acl.identityreference
        ...
        Write-TierModelLog -Level Info -Message "DmsaAclFdPlanningComplete" -Data @{
            TotalActions = $actions.Count
            CreateActions = $createActions
            ExistingAcls = $existingAclCount
            ValidationErrors = $planErrors.Count
        ...
        Write-TierModelLog -Level Error -Message "dMSA ACL Fd planning failed" -Data @{
            Exception = $_.Exception.Message
            CorrelationId = $CorrelationId
        } | Out-Null
