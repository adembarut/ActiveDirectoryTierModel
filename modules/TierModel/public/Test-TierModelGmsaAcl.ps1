function Test-TierModelGmsaAcl {
    <#
    .SYNOPSIS
    Audit TierModel gMSA ACL delegations against configuration.
    
    .DESCRIPTION
    Performs drift detection for group managed service account ACL delegations by
    ...
    Write-TierModelLog -Level Info -Message "GmsaAclAuditStart" -Data @{
    ...
    try {
        ...
        if (-not ($Config.PSObject.Properties.Name -contains 'gmsaAclDelegations') -or -not $Config.gmsaAclDelegations) {
        ...
        if (-not $Silent) {
            Write-Host "Auditing gMSA ACL delegations..." -ForegroundColor Cyan
        }
        ...
        $serviceAccountGuid = [Guid](& $resolveAclGuid 'msDS-GroupManagedServiceAccount')
        ...
        foreach ($group in $delegationGroups.Values) {
            ...
            if (-not $Silent) {
                Write-Host "Checking gMSA ACL Delegation: $identifier" -ForegroundColor Cyan
            }
            ...
            if ($missingExpected.Count -eq 0 -and $unexpectedAces.Count -eq 0) {
                ...
                $findings += [PSCustomObject]@{
                    ...
                    Identifier = $identifier
                    Property = 'ACEs'
                    ExpectedValue = 'Both expected gMSA ACEs present'
                    ActualValue = 'Matched'
            ...
            if ($unexpectedAces.Count -gt 0) {
                ...
                $findings += [PSCustomObject]@{
                    ...
                    Identifier = $identifier
                    Property = 'ACEs'
                    ExpectedValue = 'Only configured gMSA ACEs present'
        ...
        if (-not $Silent -and -not $SuppressSummary) {
            Write-Host "`n=== gMSA ACL Audit Summary ===" -ForegroundColor Blue
        ...
        Write-TierModelLog -Level Info -Message "GmsaAclAuditComplete" -Data @{
        ...
        Write-TierModelLog -Level Error -Message "gMSA ACL audit failed" -Data @{
