function Test-TierModelDmsaAcl {
    <#
    .SYNOPSIS
    Audit TierModel dMSA ACL delegations against configuration.
    
    .DESCRIPTION
    Performs drift detection for delegated managed service account ACL delegations by
    ...
    Write-TierModelLog -Level Info -Message "DmsaAclAuditStart" -Data @{
    ...
    try {
        ...
        if (-not ($Config.PSObject.Properties.Name -contains 'dmsaAclDelegations') -or -not $Config.dmsaAclDelegations) {
        ...
        $serviceAccountGuid = [Guid]$dmsaGuid
        ...
        foreach ($group in $delegationGroups.Values) {
            ...
            if (-not $Silent) {
                Write-Host "Checking dMSA ACL Delegation: $identifier" -ForegroundColor Cyan
            }
            ...
            if ($missingExpected.Count -eq 0 -and $unexpectedAces.Count -eq 0) {
                ...
                $findings += [PSCustomObject]@{
                    ...
                    Identifier = $identifier
                    Property = 'ACEs'
                    ExpectedValue = 'Both expected dMSA ACEs present'
                    ActualValue = 'Matched'
            ...
            if ($unexpectedAces.Count -gt 0) {
                ...
                $findings += [PSCustomObject]@{
                    ...
                    Identifier = $identifier
                    Property = 'ACEs'
                    ExpectedValue = 'Only configured dMSA ACEs present'
        ...
        if (-not $Silent -and -not $SuppressSummary) {
            Write-Host "`n=== dMSA ACL Audit Summary ===" -ForegroundColor Blue
        ...
        Write-TierModelLog -Level Info -Message "DmsaAclAuditComplete" -Data @{
        ...
        Write-TierModelLog -Level Error -Message "dMSA ACL audit failed" -Data @{
