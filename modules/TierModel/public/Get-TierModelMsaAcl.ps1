function Get-TierModelMsaAcl {
    <#
    .SYNOPSIS
    Analyze TierModel MSA ACL delegation requirements and generate deployment plan.
    ...
    .PARAMETER Config
    TierModel configuration object containing MSA ACL delegation definitions.
    ...
    Write-TierModelLog -Level Info -Message "MsaAclPlanningStart" -Data @{
    ...
    try {
        ...
        Write-TierModelLog -Level Info -Message "Analyzing MSA ACL delegations" -Data @{
        ...
        foreach ($ouPath in $uniqueOUs) {
            try {
                ...
                $planErrors += @{
                    ...
                    Message = "Target OU not found: '$ouPath'. Use -OusOnly to deploy the OUs, or use -FullDeployment -IncludeMsa."
        ...
        foreach ($group in $uniqueGroups) {
            try {
                ...
                $planErrors += @{
                    ...
                    Message = "Delegation group '$group' not found. Use -GroupsOnly to deploy the Groups, or use -FullDeployment -IncludeMsa."
        ...
        foreach ($acl in $delegations) {
            try {
                ...
                if ($needsApplication) {
                    ...
                    Write-TierModelLog -Level Info -Message "MSA ACL delegation needs application" -Data @{
                ...
                $planErrors += @{
                    ...
                    Message = "Failed to analyze MSA ACL delegation: $($_.Exception.Message)"
                    Context = @{
                        IdentityReference = $acl.identityreference
        ...
        Write-TierModelLog -Level Info -Message "MsaAclPlanningComplete" -Data @{
            TotalActions = $actions.Count
            CreateActions = $createActions
            ValidationErrors = $planErrors.Count
        ...
        Write-TierModelLog -Level Error -Message "MSA ACL planning failed" -Data @{
