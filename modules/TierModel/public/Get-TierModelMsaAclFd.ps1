function Get-TierModelMsaAclFd {
    <#
    .SYNOPSIS
    Analyze TierModel MSA ACL delegation requirements and generate deployment plan (Full Deployment variant).
    
    .DESCRIPTION
    Examines standalone managed service account ACL delegation configuration and current
    ...
    try {
        ...
        if (-not ($Config.PSObject.Properties.Name -contains 'msaAclDelegations') -or
            -not $Config.msaAclDelegations -or
            $Config.msaAclDelegations.Count -eq 0) {
        ...
        $delegations = @($Config.msaAclDelegations)
        
        Write-TierModelLog -Level Info -Message "Analyzing MSA ACL delegations (Full Deployment)" -Data @{
            TotalAclsInConfig = $delegations.Count
            ServiceAccountClass = 'msDS-ManagedServiceAccount'
            CorrelationId = $CorrelationId
        } | Out-Null
        ...
        foreach ($acl in $delegations) {
            try {
                    ...
                    try {
                        ...
                        if ($existingAcl) {
                            ...
                            if (-not $Silent) {
                                ...
                                Write-Host "  ✅ MSA ACL Exists: $identityReference -> $ouName" -ForegroundColor Green
                            }
                            
                            Write-TierModelLog -Level Info -Message "MSA ACL delegation already exists with exact match (Fd)" -Data @{
                        ...
                        Write-TierModelLog -Level Debug -Message "Could not read MSA ACL, will plan create action" -Data @{
                            TargetOUPath = $targetOUPath
                            IdentityReference = $identityReference
                ...
                if ($needsApplication) {
                    ...
                    Write-TierModelLog -Level Info -Message "MSA ACL delegation needs application (Fd)" -Data @{
                ...
                $planErrors += @{
                    Timestamp = Get-Date
                    Category = 'Processing'
                    Code = 'MsaAclFdAnalysisFailed'
                    Message = "Failed to analyze MSA ACL delegation (Fd): $($_.Exception.Message)"
                    Context = @{
                        IdentityReference = $acl.identityreference
        ...
        Write-TierModelLog -Level Info -Message "MsaAclFdPlanningComplete" -Data @{
            TotalActions = $actions.Count
            CreateActions = $createActions
            ExistingAcls = $existingAclCount
            ValidationErrors = $planErrors.Count
        ...
        Write-TierModelLog -Level Error -Message "MSA ACL Fd planning failed" -Data @{
            Exception = $_.Exception.Message
            CorrelationId = $CorrelationId
        } | Out-Null
